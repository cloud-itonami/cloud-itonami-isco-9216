(ns fisheryaquaculture.actor
  "FisheryAquacultureActor — the ISCO-08 9216 fishery/aquaculture-site
  scheduling/logistics coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one site-
  ...
  cloud-itonami-isco-9212's livestockfarm.actor for the elementary-
  occupation outdoor-labour hazard-domain shape, extended with a second,
  ...
  The unconditional invariant: the Fishery and Aquaculture Labourer
  Advisor can never directly commit an operating record the
  FisheryAquacultureGovernor refuses, and can never make this actor
  finalize a fishery/aquaculture-work-execution decision (e.g. authorizing
  a water-based operation to proceed) or a site-safety-clearance decision
  (e.g. declaring a site cleared for safety), and can never override a
  site safety supervisor's judgment — every commit-record! call is gated
  behind `:decide`, and both scope-excluded decision classes are a
  permanent hard block regardless of disposition path. This actor
  coordinates SITE SCHEDULING/LOGISTICS ONLY — it never performs fishery
  or aquaculture work itself and never makes a site-safety-clearance
  decision itself."
...
(defn build-graph
  "Build a compiled FisheryAquacultureActor graph. `store` implements
  `fisheryaquaculture.store/Store`. `advisor` implements
  `fisheryaquaculture.advisor/Advisor` (defaults to `mock-advisor`).
